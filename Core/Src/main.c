...
//#define SRAM_BANK_ADDR      ((uint32_t)0)
...
#define HEADBAND_HEIGHT                 64
  ...
  {
    BSP_LCD_DisplayStringAt(0, 65, (uint8_t *)"  AUDIO CODEC OK  ", LEFT_MODE);
  }
  else
  {
    BSP_LCD_SetTextColor(LCD_COLOR_RED);
    BSP_LCD_DisplayStringAt(0, 65, (uint8_t *)" AUDIO CODEC FAIL ", LEFT_MODE);
    BSP_LCD_DisplayStringAt(0, 80, (uint8_t *)" Try to reset board ", LEFT_MODE);
    while(1);
  }
  AUDIO_Start((uint32_t *)AUDIO_SRC_FILE_ADDRESS, (uint32_t)AUDIO_FILE_SIZE);
  BSP_LCD_DisplayStringAt(0, 80, (uint8_t *)"  PLAYING...", LEFT_MODE);
  /* USER CODE END 2 */
